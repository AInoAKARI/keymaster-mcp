...
exports.renderDropForm = renderDropForm;
exports.createDropServer = createDropServer;
exports.openBrowser = openBrowser;
exports.openFlyIntakeTunnel = openFlyIntakeTunnel;
...
function renderDropForm(service, keyName) {
...
async function readFormValue(request) {
    ...
    if (!value || value.length > MAX_BODY_BYTES)
        throw new Error("Secret value is required");
    return value;
}
function validatedKeymasterBaseUrl(value) {
...
async function requestWriteGrant(config) {
    ...
    if (!response.ok)
        throw new Error(`Keymaster rejected intake preflight (HTTP ${response.status})`);
    const body = (await response.json());
    if (typeof body.grant !== "string" || !body.grant)
        throw new Error("Keymaster returned no intake grant");
    return body.grant;
}
async function storeSecret(config, writeGrant, value) {
    ...
    if (!response.ok)
        throw new Error(`Keymaster rejected the write (HTTP ${response.status})`);
}
async function createDropServer(config) {
    validateName(config.service, "service");
    validateName(config.keyName, "key name");
    if (!config.token)
        throw new Error("Keymaster authentication is unavailable");
    ...
    const server = (0, node_http_1.createServer)(async (request, response) => {
        ...
        if (settled)
            return reply(response, 410, "This key post has already self-destructed");
        if (request.headers.host !== expectedHost)
            return reply(response, 403, "Host rejected");
        if (request.headers.origin !== expectedOrigin)
            return reply(response, 403, "Origin rejected");
        settled = true;
        try {
...
function openBrowser(url) {
    ...
    const child = (0, node_child_process_1.spawn)(command, args, {
        detached: true,
        env: (0, config_js_1.environmentWithoutKeymasterCredentials)(),
        stdio: "ignore",
        windowsHide: true,
    });
...
async function reserveLoopbackPort() {
    const probe = (0, node_http_1.createServer)();
    await new Promise((resolve, reject) => {
...
async function openFlyIntakeTunnel() {
    const localPort = await reserveLoopbackPort();
    const executable = process.platform === "win32" ? "flyctl.exe" : "flyctl";
    const child = (0, node_child_process_1.spawn)(executable, ["proxy", `${localPort}:${FLY_INTAKE_PORT}`, "-a", FLY_APP, "-b", "127.0.0.1", "-q"], {
        env: (0, config_js_1.environmentWithoutKeymasterCredentials)(),
        stdio: "ignore",
        windowsHide: true,
    });
...
async function main() {
    ...
    try {
        ...
        if (service === "npm")
            openBrowser("https://www.npmjs.com/settings/akari-os/tokens");
